...
export default class Account extends React.Component {
        ...
        return (
                <div className="statistics-main">
                    {Object.keys(this.state.data["progress"]).map((subject, index) => 
                        {
                        var p = Math.round(this.state.data["myscore"][subject].split(", ").reduce((a,b)=>parseInt(a)+parseInt(b)) * 100/120)
                        p = p >= 85?"Senior":p>=65?"Medium":p>=25?"Junior":"Beginner"
                        return (
                            <div className="st-subject">
                                <div className="st-fill">
                                    <Circle
                                        progress={Math.round(this.state.data["myscore"][subject].split(", ").reduce((a,b)=>parseInt(a)+parseInt(b)) * 100/120)} animated={true} lineWidth="35"
                                        ...
                                        <div className="st-row">
                                            <div className="s">
                                                <div className="st-icon1">
                                                    <img className="img1" src="https://image.flaticon.com/icons/svg/238/238616.svg"/>
                                                </div>
                                                <label className="st-big">
                                                    {Math.round(this.state.data["myscore"][subject].split(", ").reduce((a,b)=>parseInt(a)+parseInt(b)))}
                                                </label>
                                                <label className="st-medium">Тесты</label>
                                            </div>
                                            <div className="s">
                                                ...
                                                <label className="st-medium">Экзамен</label>
                                            </div>
                                        </div>
                                    </div>
                                </div>
    ...
    downloadNode = () => {
        ...
        var course_name = document.getElementById("canvas-course").textContent = "Курс " + obj[this.state.currentIndex]["course"]
        var course_level = document.getElementById("canvas-level").textContent = "Уровень " + obj[this.state.currentIndex]["level"]
        console.log(node)
        setTimeout(() => {
                ...
                <div className="Account">
                    ...
                    <div className="account-nav">
                            ...
                            <div className="account-nav-first">
                                <img src="https://image.flaticon.com/icons/svg/149/149071.svg" className="account_profile_picture"/>
                                <div className="account-nav-first-control">
                                    <label>Student</label>
                                    <span onClick={()=>firebase.auth().signOut()}>{language[0][this.state.language]["out"]}</span>
                                </div>
                            </div>
                    </div>
                    ...
                    <div className="account_main">
                        ...
                        <div className="account-footer">
                            <div className="account-write-us">
                                <img className="foreground" src="https://image.flaticon.com/icons/svg/281/281769.svg"/>
                                <div>
                                    <label className="account-contact-title">TECEDU</label>
                                    <label className="account-contact-subtitle">{language[0][this.state.language]["write_us"]}</label>
                                     <a className="post" target="_blank" aria-label="Our Email" rel="noopener noreferrer" href="mailto:tecedu@gmail.com">
                                        <label className="post">tecedu@gmail.com</label>
                                    </a>
                                </div>
                            </div>
                            <div className="account-settings">
                                <img style={{height: "calc(100% - 25px)", top: 12.5}} className="foreground" src="https://image.flaticon.com/icons/svg/148/148912.svg"/> 
                                <div>
                                    <label className="acc-settings-title">{language[0][this.state.language]["settings"]}</label>
                                    <label className="acc-settings-subtitle">Ваша информация</label>
                                    <label className="acc-settings-element">Email: {firebase.auth().currentUser.email}</label>
                                    <label className="acc-settings-element phio">Ф.И.О: {this.state.data.name != undefined ? this.state.data.name[0] + " " + this.state.data.name[1] : "Не указано"}</label>
                                    <button className="acc-change-psw" onClick={()=>{
                                        ...
                                        firebase.auth().sendPasswordResetEmail(firebase.auth().currentUser.email).then(function () {
                                            ...
                                            Alert.warning("Новый пароль отправлен на вашу почту" , {
                                                position: 'top-right',
                                                effect: 'slide',
                                                beep: false,
                                                timeout: 3000,
                        ...
                        <div className="Statistics">
                            <label className="title">Ваша статистика</label>
                            {this.statistics()}
                        </div>
                        {
                            obj[0] == undefined ?  
                                <div className="certificates">
                                    <label className="title">Мои сертификаты</label>
