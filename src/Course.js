...
const header_imgs = {"Добыча нефти с поверхности земли":require("./imgs/first.png"),"JavaScript": require("./imgs/second.png"),"Python": require("./imgs/third.png")}
...
export default class Course extends  React.Component {
    ...
    showTest = (finished) => {
        var text = finished ? "Вы хотите сейчас написать контрольную работу по всем секциям? У вас не будет возможности заново сдать этот тест" : "Вы хотите сейчас написать контрольную работу по данной секций? У вас не будет возможности заново сдать этот тест"
                        ...
                        if (this.state.exam != undefined) {
                            if (this.state.exam[this.state.course] == -1 || this.state.exam[this.state.course] == "-1" ) {
                                if (this.checkAllExamsAndTasks()) {
                                    ...
                                    alert("Вы должны закончить все задания")
                                }
                            } else {
                                window.location.href = "/finished/" + this.state.course
                            }
